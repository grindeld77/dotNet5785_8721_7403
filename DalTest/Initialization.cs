        ...
        CallType[] callTypes =
{
    ...
    "car crash on highway"
        ...
        for (int i = 0; i < 45; i++)
        {
            ...
            if (Max < s_dal.Config.Clock)
                callStatus = CallStatus.Expired;
            else if (Open < s_dal.Config.Clock)
                callStatus = CallStatus.InProgress;

            Call callToAdd = new Call(id, callTypes[i], callDescriptions[i], callAddresses[i], callLatitudes[i], callLongitudes[i], Open, Max, callStatus);
            if (callToAdd != null)
            {
        ...
        for (int i = 45; i < 50; i++) //5 that expird 
        {
            ...
            Call callToAdd = new Call(id, callTypes[i], callDescriptions[i], callAddresses[i], callLatitudes[i], callLongitudes[i], Open, Max, CallStatus.Expired);
        ...
        for (int i = 0; i < 15; i++) //15 not allcoated call
        {
            int id = 0; //will be generated by the create method in CallImplementation
            DateTime TempTime = s_dal!.Config.Clock.AddDays(-s_rand.Next(1, 10)); //time initialized to system time minus random days
            Call callToAdd = new Call(id, 0, null, "", 0, 0, TempTime, null, CallStatus.Closed);
            if (callToAdd != null)
            {
                s_dal!.Call.Create(callToAdd);
            }
